        ...
        {data.map((item, index) => {
          return (
            checkVisibility(item.isVisibleFunction) && (
              <>
                <TableRow hover selected={checkableRows && checkedContentItems?.length > 0 && checkedContentItems.findIndex((checkedRow: ContentTableRow) => item.id === checkedRow.id) !== -1} key={index} onContextMenu={onContextMenu ? (event) => onContextMenu(event, item, checkedContentItems) : (e) => e.preventDefault()}>
                  {checkableRows && (
...
export const TableDataCell = styled(MuiTableCell) <{ type?: ContentType; clickable?: string; colors?: color }>`
  position: relative;
  z-index: 0;
