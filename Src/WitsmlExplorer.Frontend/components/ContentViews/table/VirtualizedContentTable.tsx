...
const Row = memo(({ data, index, style }: RowProps) => {
  ...
  return (
    <RowWrapper className="bin" key={"Row-" + index} style={style} columns={columns} checkableRows={checkableRows} width={width} compactMode={isCompactMode}>
      <TableRow
        key={"TableRow-" + index}
        hover
...
const innerGridElementType = forwardRef<HTMLDivElement, any>(({ children, ...rest }: any, ref) => {
  ...
  return (
    <div ref={ref} style={rest.style} className="bino">
      <TableHeader>
...
const TableDataCell = styled(MuiTableCell)<{ clickable?: string; colors: color }>`
